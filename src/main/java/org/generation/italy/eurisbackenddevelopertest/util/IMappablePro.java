package org.generation.italy.eurisbackenddevelopertest.util;
...
public interface IMappablePro {
	...
	/**
	 * Questo metodo permette la conversione automatica da oggetto a mappa
	...
	default void fromMap( Map<String, String> map ) {
		...
		for ( Method metodo : metodi ) {
			...
			if ( nomeMetodo.startsWith( "set" ) ) {
				...
				// se il tipo è primitivo o se non lo è
				// c'è un'unica particolarità: il char
				// Nella stringa contenuta nel valore della mappa il char
				// è salvato così: "A"
				// a me serve invece così: 'A'
				try {
					if ( tipoParametro.equals( char.class ) ) {
						...
						// se il parametro è int lui andrà a cercare il tipo boxato
						...
						for ( Method metodoBoxato : metodiBoxati ) {
							...
							if ( tipoParametro.equals( boolean.class ) ) {
								metodo.invoke( this, valore.equals( "1" ) || valore.equalsIgnoreCase( "true" ) );
							} else if ( metodoBoxato.getName().startsWith( "parse" ) &&
